                    ...
                    <c:forEach var="entity" items="${result.data}" varStatus="num">
                        <tr>
                            ...
                            <td>
                                <c:if test="${entity.status == 0}">
                                    <a href="javascript:"
                                       data-url="/stockIncomeBill/audit.do?id=${entity.id}"
                                       class="btn_audit" data-obj="stockIncomeBill"
                                    >审核</a>
                                    <a href="/stockIncomeBill/input.do?id=${entity.id}">编辑</a>
                                    <a href="javascript:"
                                       data-url="/stockIncomeBill/delete.do?id=${entity.id}"
                                       class="btn_delete" data-obj="stockIncomeBill"
                                    >删除</a>
                                </c:if>
                                <c:if test="${entity.status == 1}">
                                    <a href="/stockIncomeBill/show.do?id=${entity.id}">查看</a>
                                </c:if>
                            </td>
                        </tr>
                    </c:forEach>
